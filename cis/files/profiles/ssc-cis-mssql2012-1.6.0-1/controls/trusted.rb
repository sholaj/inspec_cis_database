# MSSQL 2012 InSpec Controls - trusted.rb
# CIS Microsoft SQL Server 2012 Benchmark v1.6.0
# Profile version: 1.6.0 (see inspec.yml)
#
# This profile implements CIS Microsoft SQL Server 2012 Benchmark controls
# for enterprise compliance scanning.
#
# Control ID Format: mssql-2012-X.XX
...
control 'mssql-2012-1.01' do
  ...
  describe sql.query("SELECT CASE WHEN CAST(SERVERPROPERTY('ProductLevel') AS VARCHAR(10)) IN ('SP1', 'SP2', 'SP3', 'SP4', 'RTM') THEN 'CHECK_VERSION' ELSE 'UNKNOWN' END AS results") do
    its('rows.first.results') { should_not eq 'UNKNOWN' }
  end
end

control 'mssql-2012-1.02' do
  impact 1.0
  ...
  describe sql.query("SELECT CASE WHEN CAST(SERVERPROPERTY('ProductVersion') AS VARCHAR(20)) >= '11.0.2100' THEN 'COMPLIANT' ELSE 'NOT COMPLIANT' END AS results") do
    its('rows.first.results') { should eq 'COMPLIANT' }
  end
end

control 'mssql-2012-1.03' do
  impact 0.7
  ...
  describe sql.query("SELECT CASE WHEN COUNT(*) = 0 THEN 'COMPLIANT' ELSE 'NOT COMPLIANT' END AS results FROM sys.databases WHERE name IN ('AdventureWorks', 'AdventureWorksDW', 'AdventureWorksLT', 'Northwind', 'pubs')") do
    its('rows.first.results') { should eq 'COMPLIANT' }
  end
end
