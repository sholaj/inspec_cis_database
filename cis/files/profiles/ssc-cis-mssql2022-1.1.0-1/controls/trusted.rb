# MSSQL 2022 InSpec Controls - trusted.rb
# CIS Microsoft SQL Server 2022 Benchmark v1.1.0
# Profile version: 1.1.0 (see inspec.yml)
#
# This profile implements CIS Microsoft SQL Server 2022 Benchmark controls
# for enterprise compliance scanning.
#
# Control ID Format: mssql-2022-X.XX
# Tags: cis (control number), cis_level (1 or 2), severity (critical/high/medium/low)
#
# MSSQL 2022 New Features Checked:
...
# CIS Microsoft SQL Server 2022 Benchmark v1.0.0 - Section 1
# ==============================================================================

control 'mssql-2022-1.01' do
  impact 1.0
  ...
  describe sql.query("SELECT CASE WHEN CAST(SERVERPROPERTY('ProductLevel') AS VARCHAR(10)) IN ('SP1', 'SP2', 'SP3', 'RTM', 'CU') THEN 'CHECK_VERSION' ELSE 'UNKNOWN' END AS results") do
    its('rows.first.results') { should_not eq 'UNKNOWN' }
  end
end

control 'mssql-2022-1.02' do
  impact 1.0
  ...
  # MSSQL 2022 RTM starts at version 16.0.1000
  describe sql.query("SELECT CASE WHEN CAST(SERVERPROPERTY('ProductVersion') AS VARCHAR(20)) >= '16.0.1000' THEN 'COMPLIANT' ELSE 'NOT COMPLIANT' END AS results") do
    its('rows.first.results') { should eq 'COMPLIANT' }
  end
end

control 'mssql-2022-1.03' do
  impact 0.7
  ...
  # MSSQL 2022 is ProductMajorVersion 16
  describe sql.query("SELECT CASE WHEN CAST(SERVERPROPERTY('ProductMajorVersion') AS INT) >= 16 THEN 'COMPLIANT' ELSE 'NOT COMPLIANT' END AS results") do
    its('rows.first.results') { should eq 'COMPLIANT' }
  end
end

control 'mssql-2022-1.04' do
  impact 0.7
  ...
  describe sql.query("SELECT CASE WHEN COUNT(*) = 0 THEN 'COMPLIANT' ELSE 'NOT COMPLIANT' END AS results FROM sys.databases WHERE name IN ('AdventureWorks', 'AdventureWorksDW', 'AdventureWorksLT', 'Northwind', 'pubs', 'WideWorldImporters', 'WideWorldImportersDW')") do
    its('rows.first.results') { should eq 'COMPLIANT' }
  end
end
...
# CIS Microsoft SQL Server 2022 Benchmark v1.0.0 - Section 2
# ==============================================================================

control 'mssql-2022-2.01' do
  impact 1.0
...
# ==============================================================================
# Section 2.19-2.21: SQL Server 2022 Specific Controls
# New features in MSSQL 2022 that require security configuration
# ==============================================================================

control 'mssql-2022-2.19' do
  impact 0.7
  title "Ensure 'Ledger' Database Feature is Configured Securely"
  desc 'SQL Server 2022 Ledger provides tamper-evidence for database tables. Verify ledger configuration if used.'

  tag cis: '2.19'
  tag cis_level: 2
  tag severity: 'high'
  tag mssql_2022_specific: true

  # Check if ledger databases exist and have proper digest storage
  describe sql.query("SELECT CASE WHEN NOT EXISTS (SELECT 1 FROM sys.databases WHERE is_ledger_on = 1) OR EXISTS (SELECT 1 FROM sys.database_ledger_digest_locations WHERE is_current = 1) THEN 'COMPLIANT' ELSE 'REVIEW_LEDGER_CONFIG' END AS results") do
    its('rows.first.results') { should be_in ['COMPLIANT', 'REVIEW_LEDGER_CONFIG'] }
  end
end

control 'mssql-2022-2.20' do
  impact 0.7
  title "Ensure 'Query Store' is Configured Securely"
  desc 'SQL Server 2022 Query Store has enhanced features. Ensure proper configuration for security and performance.'
  ...
  # Verify Query Store is either disabled or properly configured
  describe sql.query("SELECT CASE WHEN NOT EXISTS (SELECT 1 FROM sys.databases WHERE is_query_store_on = 1 AND database_id > 4) OR NOT EXISTS (SELECT 1 FROM sys.databases d WHERE is_query_store_on = 1 AND database_id > 4 AND NOT EXISTS (SELECT 1 FROM sys.database_query_store_options WHERE desired_state_desc != 'OFF')) THEN 'COMPLIANT' ELSE 'REVIEW_QS_CONFIG' END AS results") do
    its('rows.first.results') { should be_in ['COMPLIANT', 'REVIEW_QS_CONFIG'] }
  end
end

control 'mssql-2022-2.21' do
  impact 0.7
  title "Ensure 'Contained Availability Groups' are Configured Securely"
  desc 'SQL Server 2022 contained availability groups require proper security configuration.'
  ...
  # Check contained AG configuration
  describe sql.query("SELECT CASE WHEN NOT EXISTS (SELECT 1 FROM sys.availability_groups WHERE is_contained = 1) THEN 'COMPLIANT' ELSE 'REVIEW_CONTAINED_AG' END AS results") do
    its('rows.first.results') { should be_in ['COMPLIANT', 'REVIEW_CONTAINED_AG'] }
  end
end

# ==============================================================================
# Section 3: Authentication and Authorization
# CIS Microsoft SQL Server 2022 Benchmark v1.0.0 - Section 3
# ==============================================================================

control 'mssql-2022-3.01' do
  impact 0.7
...
# CIS Microsoft SQL Server 2022 Benchmark v1.0.0 - Section 4
# ==============================================================================

control 'mssql-2022-4.01' do
  impact 1.0
...
# CIS Microsoft SQL Server 2022 Benchmark v1.0.0 - Section 5
# ==============================================================================

control 'mssql-2022-5.01' do
  impact 0.7
...
# CIS Microsoft SQL Server 2022 Benchmark v1.0.0 - Section 6
# ==============================================================================

control 'mssql-2022-6.01' do
  impact 0.7
...
# CIS Microsoft SQL Server 2022 Benchmark v1.0.0 - Section 7
# ==============================================================================

control 'mssql-2022-7.01' do
  impact 1.0
...
control 'mssql-2022-7.06' do
  impact 1.0
  title 'Ensure TLS 1.2 or Higher is Used'
  desc 'Older TLS versions have known vulnerabilities. SQL Server 2022 supports TLS 1.3.'
...
# CIS Microsoft SQL Server 2022 Benchmark v1.0.0 - Section 8
# ==============================================================================

control 'mssql-2022-8.01' do
  impact 0.7
